...
printf "This script is a tool to install APCU  for ltanmp \n"
printf "\n"
...
apcuv=`grep "apcu.so" /usr/local/php55/etc/php.ini`

if echo $apcuv | grep -q "apcu.so*";then
sed -i '/extension = "apcu.so"/d' /usr/local/php55/etc/php.ini

/etc/init.d/php-fpm55 stop
/etc/init.d/php-fpm55 start
...
phpv=`/usr/local/php55/bin/php -v`

if echo $phpv | grep -q "PHP 5.2.*" || echo $phpv | grep -q "PHP 5.3.*" || echo $phpv | grep -q "PHP 5.4.*" || echo $phpv | grep -q "PHP 5.5.*";then

	echo "PHP Ver OK ! "

  else
	echo "PHP Ver Error Can't install  APCu !"
	exit 1
fi
...
/usr/bin/phpize
./configure --enable-apcu --with-php-config=/usr/local/php55/bin/php-config
make && make install


sed -ni '1,/;eaccelerator/p;/;ionCube/,$ p' /usr/local/php55/etc/php.ini
sed -i '/extension = "apcu.so"/d' /usr/local/php55/etc/php.ini
sed -i '/;eaccelerator/a extension = "apcu.so"' /usr/local/php55/etc/php.ini
