...
install_mysql() {
    ...
    cd ${current_dir}/src
    tar -zxvf ${ltnmp_mysql}.tar.gz
    cd ${ltnmp_mysql}

    cmake -DCMAKE_INSTALL_PREFIX=/usr/local/mysql -DEXTRA_CHARSETS=all -DDEFAULT_CHARSET=utf8mb4 -DDEFAULT_COLLATION=utf8mb4_unicode_ci -DWITH_READLINE=1 -DWITH_SSL=bundled -DWITH_ZLIB=system -DWITH_EMBEDDED_SERVER=1 -DENABLED_LOCAL_INFILE=1 -DCMAKE_EXE_LINKER_FLAGS='-ljemalloc' -DWITH_SAFEMALLOC=OFF
    make && make install
    ...
    cp ${current_dir}/lib/conf/my.cnf /etc/my.cnf
