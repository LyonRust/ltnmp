...
install_mariadb10()
{
    ...
    sed '/skip-external-locking/i\pid-file = /usr/local/mariadb/data/mariadb.pid' -i /etc/my.cnf
    sed '/skip-external-locking/i\log_error = /usr/local/mariadb/data/mariadb.err' -i /etc/my.cnf
    ...
    #sed -i 's:^datadir=$:datadir=/usr/local/mariadb/data:g' /etc/init.d/mariadb
